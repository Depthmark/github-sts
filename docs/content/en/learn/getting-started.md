...
description: Run github-sts locally and exchange your first OIDC token for a scoped GitHub installation token.
weight: 1
...
- An OIDC token from a supported issuer (e.g. GitHub Actions, Azure, GCP): see [OIDC Issuers]({{< relref "/learn/oidc-issuers" >}})
...
See [Configuration]({{< relref "/reference/configuration" >}}) for the full reference.
...
If `/ready` returns `503`, the server has not yet started serving. Check server logs.
...
> **Important:** `audience` is mandatory. The same value must be passed to `core.getIDToken(<audience>)` in the workflow that requests the OIDC token.
...
- [API Reference]({{< relref "/reference/api" >}}): full request/response schema, error codes, Go client library
- [Configuration]({{< relref "/reference/configuration" >}}): every YAML/env var, trust policy schema, org-level scope
- [Deployment]({{< relref "/operations/deployment" >}}): Docker and Helm
- [Troubleshooting]({{< relref "/operations/troubleshooting" >}}): common errors and how to fix them
