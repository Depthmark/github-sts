...
| **`rotation.strategy: rate_limit_aware` keeps using a drained member** | The order only changes for a member whose installation is known to be low. Check `githubsts_app_pool_selection_total{outcome="skipped_rate_limited"}` and the member's `githubsts_github_rate_limit_remaining_percent`. If the gauge reads full while the member is drained, the reading is stale or missing: check `githubsts_github_rate_limit_observed_timestamp` against `metrics.rate_limit_poll_force_counted_interval` (a gap wider than it means the poller isn't forcing counted re-reads — confirm it's enabled and not set to an unreasonably long value), `githubsts_github_rate_limit_probe_total`, and the startup log for the warning about `metrics.rate_limit_poll_enabled` being off. A bucket that only client tokens spend is never visible. See [Configuration]({{< relref "/reference/configuration#app-pools-multi-instance-rate-limit-rotation" >}}). |
