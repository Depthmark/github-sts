...
The default `rotation.strategy` is `round_robin`. `rate_limit_aware` is opt-in: on each request it moves a member whose installation is known to be below `rotation.min_remaining_pct` to the back of the order, without removing it. Its readings come from the rate limit poller, which every replica runs on a timer whatever traffic it routes, and from github-sts's own installation-token calls. Only responses that spent a request are trusted, because for installation tokens GitHub answers uncounted requests (`GET /rate_limit`, `304 Not Modified`) with an unused bucket. A reading can lag behind a fast drain, and a bucket that only client tokens spend is never visible, so reactive failover on a live `403` stays the backstop. The baseline reachability filter above applies to both strategies.
