...
| `permissions` | `map[string]string` | GitHub App permissions. A **ceiling**, not a fixed grant: a caller may request a subset, or a lower level, and receive a token limited to that. The accepted level depends on the permission: most take `read` or `write`, four also take `admin`, and a few are read-only or write-only. |
