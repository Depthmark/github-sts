...
By default, github-sts round-robins across a pool's instances, skips any instance the reachability prober currently reports down, and fails over to the next instance when the one it tried returns a retryable error. The rotation takes one turn per token exchange: the exchange's final token mint advances it, and the target lookup and policy reads before that mint start on the member the mint will use. One exchange therefore stays on one member, and mints spread evenly across the pool whatever its size. Retries within one request walk consecutive members. Retryable means a network/timeout error, a 5xx response, or a 403 that carries a rate-limit signal (`Retry-After` or `X-RateLimit-Remaining: 0`); a 422 (the requested permissions or repositories exceed what that installation grants) or any other 4xx is not retried, since trying a different credential cannot fix a permissions mismatch. `rotation.max_attempts` bounds how many instances one request will try (default: pool size, capped at 3).
...
- `rotation.strategy` is `round_robin` (default) or `rate_limit_aware`. With `rate_limit_aware`, each request moves to the back of the order any member whose installation for the requested organization is known to be below `rotation.min_remaining_pct`, or out of requests, in a rate limit window that has not reset yet. The member is not removed: it is still tried when every other member fails. Each time a request passes it over without trying it, `githubsts_app_pool_selection_total{outcome="skipped_rate_limited"}` counts it. When every member is low, or nothing is known, the order is plain round robin.
- Quota readings for `rate_limit_aware` come only from responses that spent a request: github-sts's own installation-token calls and the rate limit poller. They can lag behind a fast drain, bounded by `metrics.rate_limit_poll_force_counted_interval`, since the poller's own conditional probe would otherwise go untrustworthy for up to a token's cache lifetime (~55 minutes), and a bucket that only client tokens spend is never visible, so reactive failover stays the backstop. See the [Metrics reference]({{< relref "/reference/metrics" >}}) for where the readings come from. github-sts logs a startup warning when `rate_limit_aware` is set while `metrics.rate_limit_poll_enabled` is off.
- `rotation.min_remaining_pct` (range `[0, 100)`) only applies to `rate_limit_aware`. With `0`, only a member with no requests left is moved back.
