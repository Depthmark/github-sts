...
La stratégie `rotation.strategy` par défaut est `round_robin`. `rate_limit_aware` est optionnelle : à chaque requête, elle place en fin d'ordre un membre dont l'installation est connue comme étant sous `rotation.min_remaining_pct`, sans le retirer. Ses relevés proviennent du sondeur de limite de débit, que chaque réplique exécute sur minuterie quel que soit le trafic qu'elle route, et des appels de github-sts faits avec ses propres jetons d'installation. Seules les réponses qui ont consommé une requête sont retenues, car pour les jetons d'installation GitHub répond aux requêtes non décomptées (`GET /rate_limit`, `304 Not Modified`) avec un compteur inutilisé. Un relevé peut être en retard sur un épuisement rapide, et un compteur dépensé uniquement par des jetons clients n'est jamais visible : la bascule réactive sur un `403` en direct reste le filet de sécurité. Le filtre d'accessibilité de base ci-dessus s'applique aux deux stratégies.
