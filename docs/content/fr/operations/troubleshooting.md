...
| Problème | Solution |
|---|---|
| **L'échange renvoie `502`** avec `code: "upstream_error"` et `githubsts_app_pool_exhausted_total` en hausse pour cette App | Toutes les instances du pool de l'App ont échoué pour cette requête, pas une seule. Vérifiez individuellement les identifiants et l'installation de chaque instance : `githubsts_github_reachable{github_app=...,github_app_instance=...}` et les lignes de journal étiquetées `instance` autour de l'échec permettent de cibler une instance précise. |
...
| **`rotation.strategy: rate_limit_aware` continue d'utiliser un membre épuisé** | L'ordre ne change que pour un membre dont l'installation est connue comme basse. Vérifiez `githubsts_app_pool_selection_total{outcome="skipped_rate_limited"}` et la jauge `githubsts_github_rate_limit_remaining_percent` du membre. Si la jauge indique un compteur plein alors que le membre est épuisé, le relevé est périmé ou absent : comparez `githubsts_github_rate_limit_observed_timestamp` à `metrics.rate_limit_poll_force_counted_interval` (un écart supérieur signifie que le sondeur ne force pas de relecture décomptée — vérifiez qu'il est activé et pas réglé sur une valeur excessive), vérifiez `githubsts_github_rate_limit_probe_total`, ainsi que le journal de démarrage pour l'avertissement indiquant que `metrics.rate_limit_poll_enabled` est désactivé. Un compteur dépensé uniquement par des jetons clients n'est jamais visible. Voir [Configuration]({{< relref "/reference/configuration#pools-dapps-rotation-multi-instances-pour-la-limite-de-débit" >}}). |
