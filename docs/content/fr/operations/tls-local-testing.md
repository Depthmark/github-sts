...
Ce guide explique comment créer une autorité de certification locale, signer un certificat serveur, démarrer github-sts en HTTPS, puis ajouter le TLS mutuel (mTLS), le tout avec `openssl` et `curl`. Aucun identifiant GitHub App réel n'est nécessaire pour vérifier la couche TLS ; l'endpoint `/health` répond sans authentification.
