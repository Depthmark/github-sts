...
class TestTrustPolicyAudience:
    """Unit tests for the audience field on trust policies."""

    def test_audience_accepted_when_matching(self):
        """Intention: verify a policy with audience stores the value correctly.

        What is being tested:
        - `TrustPolicy` can carry an `audience` field.

        Expected output:
        - The audience field is stored on the model.
        """
        policy = TrustPolicy(
        ...
        assert policy.audience == "https://management.core.windows.net/"

    def test_audience_defaults_to_none(self):
        """Intention: verify policies without audience remain backward compatible.

        What is being tested:
        - Omitting `audience` defaults to `None`.

        Expected output:
        - `policy.audience` is `None`.
        """
        policy = TrustPolicy(
            issuer="https://token.actions.githubusercontent.com",
            subject="repo:org/repo:ref:refs/heads/main",
        ...
        assert policy.audience is None
